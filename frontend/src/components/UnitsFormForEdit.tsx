...
const UnitsFormForEdit: React.FC<UnitsFormForEditProps> = ({
  value = [],
  onChange,
  //  defaultUnitImg,
  unitReferences,
  ...
  const handleUnitsChange = (index: number, name: string) => {
    const updatedUnits = [...value];
    updatedUnits[index].name = name.trim();
  ...
  const handleRemove = (unitName: string, index: number) => {
    ...
    if (originalUnit) {
      Modal.confirm({
        title: 'Delete Unit?',
        content: `The unit "${unitName}" may be used by other events. Deleting it may cause data loss. Are you sure to proceed?`,
        onOk: () => {
          const updatedUnits = [...value];
          updatedUnits.splice(index, 1);
          onChange(updatedUnits);
  ...
  useEffect(() => {
    if (value.length === 0) {
      onChange([{ id: null, name: '', img: DEFAULT_IMAGE_URL }]);
    }
  }, [value, onChange]);
          ...
          <>
              ...
              >
                ...
                >
                  ...
                  <Space>
                    ...
                    {originalUnits?.some(
                      (unit) =>
                        unit.name.trim().toLowerCase() === value[index]?.name.trim().toLowerCase(),
                    ) && (
                      <Popover
                        // content=
                        title="Caution"
                        content={
                          ...
                          >
                            <p>
                              This unit may be used by other events. Modifying or deleting it is not
                              permitted.
                            </p>
                          </div>
                        }
            ...
            <Form.Item>
              <Button
                type="dashed"
                onClick={() => {
                  add({ id: null, name: '', img: DEFAULT_IMAGE_URL });
                }}
                style={{ width: '100%' }}
                icon={<PlusOutlined />}
              >
...
export default UnitsFormForEdit;
